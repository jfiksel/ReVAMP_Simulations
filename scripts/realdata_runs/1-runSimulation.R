...
### ReVAMP with Ensemble lite sampler
set.seed(123)
revamp.seeds <- sample(1e6, nchains, replace = F)
...
revampIndPredictionsList <- function(revamp.list, test.cod, causes, burnin, thin, revamp.type) {
    revamp.thin <- lapply(revamp.list, function(x) x[seq(burnin, length(x), by = thin)])
    revamp.concat <- do.call(c, revamp.thin)
    if(revamp.type == "standard"){
        return(revampIndPredictions(revamp.concat,
                                    test.cod = test.cod,
                                    causes = causes, burnin = 1,
                                    thin = 1)$topCOD)
    } else {
        return(revampEnsembleLiteIndPredictions(revamp.concat,
                                                test.cod.mat = test.cod,
                                                causes = causes, burnin = 1,
                                                thin = 1)$topCOD)
    }
...
tariff.revamp.cod <- revampIndPredictionsList(tariff.revamp,
                                              test.cod = tariff.train.cod.test,
                                              causes = causes, burnin = burnin,
                                              thin = thin, revamp.type = "standard")
tariff.revamp.ccc <- ccc(tariff.revamp.cod, cod.truth, causes)

insilico.revamp.cod <- revampIndPredictionsList(insilico.revamp,
...
ensemble.revamp.cod <- revampIndPredictionsList(ensemble.revamp,
                                                test.cod = test.cod.mat,
                                                causes = causes, burnin = burnin,
                                                thin = thin, revamp.type = "ensemble")
